...
초기 제작 과정에서 GPT Image 2로 씬별 키비주얼 이미지를 생성했지만, Sora 2 영상 생성 단계에서는 이미지 첨부 기능이 없어 해당 이미지를 직접 첨부하지 않았다. 따라서 생성 이미지는 최종 영상의 직접 소스가 아니라 스토리보드와 톤앤매너를 검토하기 위한 사전 기획 자료로 활용하였다.
...
초기에는 4개 씬을 각각 이미지와 영상으로 제작한 뒤 편집하는 방식을 고려하였다. 그러나 영상 생성은 크레딧 소모가 크고, 본 교육 홈페이지에 있는 영상 만들기 AI기능에는 이미지를 추가할 수 없었다. 그래서 최종적으로 **Sora 2에서 4초 통합 광고 영상 1개를 생성하는 방식**을 선택하였다.
...
### 14.1 서로 다른 도구 결과물을 통합할 때 생긴 불일치는 어떻게 보정했는가?

이번 제작에서는 최종 영상을 Sora 2에서 한 번에 생성했기 때문에 실제 최종본에서는 여러 영상 클립을 편집 단계에서 직접 이어 붙이는 방식의 불일치는 크게 발생하지 않았다. 다만 제작 과정에서는 Gemini 3.1 Pro, GPT Image 2, Sora 2, TTS-1 등 서로 다른 도구를 사용했기 때문에 도구 간 결과물의 톤앤매너가 달라질 가능성을 고려하였다.

가장 큰 불일치 가능성은 다음 세 가지였다.

| 불일치 유형 | 발생 가능 문제 | 보정 전략 |
|---|---|---|
| 시각 스타일 불일치 | 이미지 생성 결과와 영상 생성 결과의 색감, 구도, 분위기가 달라질 수 있음 | 프롬프트에 warm morning sunlight, beige and wood tones, minimal premium beverage advertisement 같은 공통 스타일 키워드를 반복 사용 |
| 캐릭터 일관성 불일치 | 인물 얼굴이 씬마다 다르게 생성될 수 있음 | 얼굴 클로즈업을 제거하고 손, 노트북, 커피잔, 제품 보틀 중심으로 구성 |
| 오디오 톤 미스매치 | TTS 한국어 내레이션과 Sora 원본 영어 오디오가 겹쳐 어색해질 수 있음 | 최종본에는 TTS를 합성하지 않고 Sora 원본 오디오만 사용 |

특히 최종 영상에 한국어 TTS를 덧씌우는 방식을 검토했지만 Sora 원본 영상에 이미 영어 음성과 오디오가 포함되어 있었다. 두 음성을 동시에 사용하면 광고 메시지가 중복되고 오디오가 혼잡해질 수 있으므로 최종 영상에는 Sora 원본 오디오만 유지하였다. 이는 결과물을 무리하게 통합하기보다 최종 완성도와 청각적 일관성을 우선한 선택이다.

---

### 14.2 같은 스토리보드를 다른 도구로 제작했다면 결과가 어떻게 달라졌을 것으로 예상하는가?

같은 MORNING DROP 스토리보드를 다른 도구로 제작했다면 결과물의 강점과 약점은 달라졌을 것으로 예상한다.

| 제작 도구 | 예상 결과 | 장점 | 한계 |
|---|---|---|---|
| Sora 2 | 시네마틱하고 자연스러운 통합 광고 영상 | 짧은 영상 안에서 장면 전환과 오디오 포함 결과를 한 번에 생성 가능 | 텍스트 렌더링과 세부 브랜드명 표기가 불안정할 수 있음 |
| Pika | 짧고 감각적인 모션 중심 영상 | 이미지 기반 모션 변환에 유리하고 숏폼 스타일 제작에 적합 | 제품 광고처럼 고급스러운 질감은 결과 편차가 있을 수 있음 |
| Kling | 사실적인 움직임과 제품 질감 표현 | 액체, 손, 제품 움직임의 사실감이 좋을 가능성 | 대기열이나 크레딧 제약이 있을 수 있음 |
| Runway | 편집 친화적인 영상 소스 생성 | 이미지 기반 영상 변환과 카메라 모션 제어에 유리 | 여러 클립을 별도 편집해야 할 가능성이 큼 |

본 과제에서는 최종 제출 영상 제작 시간을 줄이고 영상과 오디오가 함께 포함된 결과물을 얻기 위해 Sora 2를 선택하였다. 만약 Pika나 Runway를 사용했다면 씬별 이미지를 각각 영상화한 뒤 CapCut에서 컷 편집을 해야 했을 가능성이 높다. 반면 Sora 2는 텍스트 프롬프트만으로 4초 통합 광고 영상을 한 번에 만들 수 있어 짧은 광고 과제에는 더 적합하다고 판단하였다.

---

### 14.3 영상 생성 크레딧이 부족하다면 제작 전략을 어떻게 바꿀 것인가?

영상 생성은 이미지 생성보다 크레딧 소모가 크기 때문에 크레딧이 부족한 상황에서는 영상 생성 횟수를 줄이고 정지 이미지와 간단한 모션 중심으로 제작 전략을 바꾸는 것이 필요하다.

본 과제에서 사용할 수 있는 전략은 다음과 같다.

1. **씬 수 축소**
   - 기존 4씬 구조를 유지하되 Sora 영상은 1개만 생성한다.
   - 실제로 본 과제에서는 4개 씬을 각각 영상화하지 않고 Sora 2에서 4초 통합 광고 영상 1개를 생성하였다.

2. **정지 이미지 활용**
   - GPT Image 2로 생성한 키비주얼 이미지를 광고 컷으로 활용하고 영상 생성은 가장 중요한 장면 1개에만 사용한다.
   - 예를 들어 커피가 얼음 위로 부어지는 Scene 2만 영상화하고 나머지는 정지 이미지로 구성할 수 있다.

3. **카메라 모션 최소화**
   - 복잡한 액션보다 slow zoom-in, soft light movement, subtle camera pan 같은 단순 모션을 사용한다.
   - 이 방식은 실패 확률을 줄이고 재생성 횟수를 줄이는 데 유리하다.

4. **텍스트 후처리 분리**
   - AI 영상 안에서 정확한 한글 자막을 생성하려고 반복 시도하지 않고 브랜드명이나 슬로건은 편집 단계에서 삽입한다.
   - 본 과제에서는 한글 텍스트 오류를 피하기 위해 Sora 프롬프트에 “no Korean text”를 명시하였다.

5. **통합 영상 생성**
   - 여러 클립을 생성하지 않고 하나의 짧은 영상 안에 전체 서사를 압축한다.
   - 본 과제의 최종 방식이 이 전략에 해당한다.

따라서 크레딧 부족 상황에서는 “여러 씬을 각각 고품질로 생성”하는 것보다 “핵심 메시지를 짧은 통합 영상으로 압축”하는 방식이 더 현실적이다.

---

### 14.4 60초 광고를 15초 버전으로 줄여야 한다면 어떻게 재구성할 것인가?

60초 광고를 15초로 줄일 때는 모든 장면을 짧게 압축하는 것이 아니라 메시지 전달에 직접 필요한 장면만 남겨야 한다. 광고의 핵심은 “문제 제시 → 제품 경험 → 해결 → 브랜드 인지” 구조이므로, 이 네 단계만 유지하고 세부 설명 장면은 제거한다.

| 60초 버전 구성 | 15초 버전 재구성 |
|---|---|
| 출근 전 피곤한 아침 상황 설명 | 0~3초: 흐릿한 아침 책상과 집중이 안 되는 분위기 |
| 직장인의 업무 스트레스 묘사 | 삭제 또는 첫 장면에 압축 |
| 제품 등장 및 콜드브루 디테일 | 3~7초: 콜드브루가 얼음잔에 부어지는 제품 경험 컷 |
| 맛과 향, 제품 원료 설명 | 삭제 또는 자막 한 줄로 축약 |
| 커피를 마신 뒤 집중 전환 | 7~11초: 밝아진 책상과 타이핑 장면 |
| 브랜드 철학 설명 | 삭제 |
| 브랜드명, 슬로건, CTA | 11~15초: 제품 히어로 컷, 브랜드명, 슬로건, CTA |

15초 버전에서는 설명보다 시각적 전환이 중요하다. 따라서 다음 요소를 우선순위로 둔다.

1. 제품이 무엇인지 즉시 알 수 있는 장면
2. 마시기 전과 후의 상태 변화
3. 브랜드명과 슬로건
4. 기억에 남는 제품 히어로 컷

MORNING DROP 광고를 15초로 줄인다면 다음과 같이 구성할 수 있다.

```text
0~3초: 흐릿한 아침 책상, 집중이 안 되는 분위기
3~7초: 콜드브루가 얼음 위로 부어지는 클로즈업
7~11초: 노트북 앞에서 빠르게 집중하는 업무 장면
11~15초: MORNING DROP 제품 컷 + 슬로건 + CTA
```

이 구조는 본 과제의 4초 광고와 같은 방향성을 가지지만, 각 장면의 여유 시간을 늘려 제품의 질감과 브랜드 메시지를 더 명확하게 전달할 수 있다.

---

### 14.5 도구 선택 기준은 무엇이었는가?

본 과제에서 도구를 선택할 때 가장 중요하게 고려한 기준은 다음 다섯 가지였다.

| 선택 기준 | 설명 | 적용 결과 |
|---|---|---|
| 과제 요구사항 충족 | 이미지, 영상, 오디오 생성 AI를 모두 활용해야 함 | GPT Image 2, Sora 2, TTS-1을 사용하거나 테스트함 |
| 결과물 완성도 | 광고처럼 보이는 시각적 품질과 브랜드 인지가 필요함 | Sora 2를 최종 영상 생성 도구로 선택 |
| 크레딧 효율성 | 영상 생성은 크레딧 소모가 크므로 생성 횟수를 줄여야 함 | 씬별 영상 4개 대신 통합 영상 1개 생성 |
| 스타일 일관성 | 짧은 광고 안에서 색감, 구도, 톤앤매너가 흔들리지 않아야 함 | warm morning sunlight, beige, minimal premium style 키워드 반복 |
| 편집 부담 최소화 | 별도 편집 없이 제출 가능한 결과물이 필요함 | Sora 2 원본 영상을 최종 제출본으로 사용 |

Gemini 3.1 Pro는 최종 미디어를 직접 만드는 도구가 아니라 기획과 프롬프트 설계에 강점이 있다고 판단하여 스토리보드 작성과 문서화에 사용하였다. GPT Image 2는 최종 영상에 직접 삽입하지는 않았지만 씬별 키비주얼을 빠르게 확인하고 광고 방향성을 점검하는 데 유용했다. Sora 2는 영상과 오디오가 포함된 통합 광고 영상을 생성할 수 있어 최종 결과물 제작에 가장 적합했다.

---

## 15. 보완 후 결론

기존 제작 문서에서는 브랜드 아이덴티티, 스토리보드, 프롬프트, 제약 대응 전략은 정리되어 있었지만 심층 인터뷰 항목 중 도구 간 불일치 보정, 다른 도구 사용 시 결과 예측, 크레딧 부족 시 전략 변경, 긴 광고를 짧은 광고로 재구성하는 방법, 도구 선택 기준에 대한 설명이 부족하였다.

이를 보완하기 위해 본 섹션에서는 실제 제작 과정에서 발생할 수 있는 멀티모달 제작 문제를 기준으로 의사결정 과정을 정리하였다. 특히 최종 영상은 Sora 2 원본을 사용했기 때문에 클립 간 편집 불일치는 크지 않았지만 키비주얼 이미지, 영상 생성 결과, TTS 테스트 음성 사이의 스타일 및 오디오 불일치 가능성을 고려하여 최종 적용 여부를 판단하였다.

결과적으로 MORNING DROP 광고 제작 과정은 단순히 AI 도구를 사용한 결과물 생성이 아니라 크레딧 제약, 도구 접근성, 스타일 일관성, 오디오 미스매치, 텍스트 렌더링 한계를 고려해 현실적으로 제작 파이프라인을 조정한 사례라고 볼 수 있다.
...
| TTS 테스트 파일 | morning_drop_voice.mp3 | 참고 자료 | 최종 영상에는 적용하지 않은 한국어 음성 테스트 파일 |
